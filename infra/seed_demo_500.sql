-- ============================================================
-- 시연용: 최근 로그인 유저에게 별 ~500개 생성
-- DAILY ~429 + DEEP ~71 = ~500 stars (500일치)
...
DECLARE
  v_user_id BIGINT;
  v_start_date DATE := (CURRENT_DATE - INTERVAL '499 days')::date;
  v_i INT;
  v_day DATE;
  v_week_start DATE;
  ...
  v_batch TEXT := 'demo500_' || to_char(clock_timestamp(), 'YYYYMMDDHH24MISS');
...
BEGIN
  ...
  RAISE NOTICE 'Inserted ~500 mock stars for user_id=% batch=%', v_user_id, v_batch;
END $$;
