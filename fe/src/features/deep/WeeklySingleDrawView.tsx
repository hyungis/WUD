            ...
            <div className="flex w-full max-w-6xl h-fit max-h-[90vh] flex-col rounded-[2rem] bg-zinc-950/40 border border-white/10 p-5 sm:p-7 backdrop-blur-3xl relative overflow-hidden">
              ...
              <div className="relative mb-5 flex flex-col shrink-0">
                ...
                <h2 className="text-xl sm:text-2xl font-bold text-white mb-2 leading-tight">
                  {surveyPageIndex === 0 ? "지난 2주 동안, 당신의 마음은 어땠나요?" : "최근 당신은 이러한 감정들을 얼마나 자주 느꼈나요?"}
                </h2>
                <p className="text-[13px] text-zinc-300 font-medium mb-4">
                  {surveyPageIndex === 0
                ...
                <div className="toolbar-popup absolute left-[calc(100%+10px)] top-1/2 -translate-y-1/2 w-[280px] bg-zinc-950 border border-white/15 p-4 rounded-2xl shadow-2xl backdrop-blur-xl z-50"
                  ...
                  <div className="grid grid-cols-6 gap-3 mb-3">
                    {PAINT_PRESET_COLORS.map((c) => (
                      <button key={c} onClick={() => selectPaintColor(c)} className={`h-9 w-9 rounded-full border border-white/20 transition-opacity ${paintColor === c ? "ring-2 ring-white ring-offset-2 ring-offset-zinc-950 opacity-100" : "opacity-85 hover:opacity-100"}`} style={{ backgroundColor: c }} />
                    ))}
                  </div>
